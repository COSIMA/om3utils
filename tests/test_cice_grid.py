...
def test_grid_ds(mom_grid, tmp_path):
    ...
    area_wrapped = xr.concat([
        ds.area.isel(nx=slice(1,None)),
        ds.area.isel(nx=0)
    ], dim='nx')

    top_row = xr.concat([
        ds.area.isel(ny=-1, nx=slice(-2,0,-1)),
...
def test_inputs_logged(cice_grid, mom_grid):
    ...
    assert (cice_grid.kmt_ds.inputfile == mom_grid.mask_path), "mask inputfile attribute incorrect ({cice_grid.kmt_ds.inputfile})"